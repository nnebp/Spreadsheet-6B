...
public static int getCellToken (String inputString, int startIndex, CellToken cellToken) {
    ...
    if (index == inputString.length()) {
        ...
        throw new IllegalArgumentException("Empty cell error");
    ...
    if (!Character.isUpperCase(ch)) {
       ...
       throw new IllegalArgumentException("Non-capitalized letters are not allowed");
    } else {
        column = ch - 'A';
        index++;
    }
    ...
    if (index == inputString.length() ) {
        ...
        throw new IllegalArgumentException("NOt a complete cell");
    }
    ...
    if (Character.isDigit(ch)) {
        ...
        throw new IllegalArgumentException("Non-numerical data");
    }
...
public static void main(String [] args)
	{
	   ...
	   System.out.println(token.getRow());
	      ...
	      System.out.println(newClass.printCellToken(token));
	   }
...
public Stack getFormula(String formula) {
    ...
    while (index < formula.length() ) {
        ...
        if (isOperator(ch)) {
            ...
            CellToken cellTokenref = new CellToken();
            index = getCellToken(formula, index, cellTokenref);
            if (cellTokenref.getRow() <= 0) {
                error = true;
                break;
            } else {
                // place the cell reference on the output stack
                returnStack.push(cellTokenref);
            }
    ...
    if (error) {
       ...
       throw new IllegalArgumentException("Bad cell token inputs");
    }
